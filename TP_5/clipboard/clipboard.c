...
static ssize_t clipboard_write(struct file *filp, const char __user *buf, size_t len, loff_t *off) {
  ...
  if (len > available_space) {
    printk(KERN_INFO "ASMN Clipboard: not enough space!!\n");
    return -ENOSPC;
  }
  ...
  *off+=len;              /* Update the file pointer */
  ...
  printk(KERN_INFO "ASMN Clipboard: write %zu bytes\n", len);
...
static ssize_t clipboard_read(struct file *filp, char __user *buf, size_t len, loff_t *off) {
  ...
  printk(KERN_INFO "ASMN Clipboard: read %d bytes\n", nr_bytes);
...
int init_clipboard_module( void )
{
  ...
  if (!clipboard) {
    ...
    if (proc_entry == NULL) {
      ret = -ENOMEM;
      vfree(clipboard);
      printk(KERN_INFO "ASMN Clipboard: No puede crear entrada en /proc..!!\n");
    } else {
      printk(KERN_INFO "ASMN Clipboard: loaded\n");
    }
  }
  return ret;
}
...
void exit_clipboard_module( void )
{
  ...
  vfree(clipboard);
  printk(KERN_INFO "ASMN Clipboard: unloaded\n");
}
...
MODULE_DESCRIPTION("TP5 Clipboard Kernel Module");
